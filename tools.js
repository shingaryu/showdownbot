	...
	importTeam(text, teams) {
		...
		for (var i = 0; i < text.length; i++) {
			...
			if (line === '' || line === '---') {
				...
				if (teams.length) {
					teams[teams.length - 1].team = this.packTeam(teams[teams.length - 1].team);
				}
				...
				if (line.substr(line.length - 1) === ')' && parenIndex !== -1) {
					...
					curSet.species = this.getTemplate(line.substr(parenIndex + 2)).species;
					line = line.substr(0, parenIndex);
					curSet.name = line;
				} else {
					curSet.species = this.getTemplate(line).species;
					curSet.name = '';
				}
